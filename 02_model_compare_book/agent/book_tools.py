...
def _git_output_path() -> str:
    return str(OUTPUT_ROOT.relative_to(RESULTS_REPO))


def sync_results(message: str = "Update model compare output") -> dict:
    """모델별 결과물(output/)을 커밋하고 원격 GitHub에 push합니다."""
...
def _with_model_metadata(metadata: dict[str, Any]) -> dict[str, Any]:
    metadata.setdefault("model", OLLAMA_MODEL)
    metadata.setdefault("output_dir", str(BOOK_OUTPUT_DIR))
    return metadata
...
def save_book_metadata(
    ...
    sync = _sync_results(f"[{OLLAMA_MODEL}] Update book metadata: {title}")
    if sync:
...
def save_outline(chapters: list[dict[str, Any]]) -> dict:
    ...
    sync = _sync_results(f"[{OLLAMA_MODEL}] Update outline ({len(chapters)} chapters)")
    if sync:
        result["git_sync"] = sync
    return result
...
def write_chapter(chapter_number: int, title: str, content: str) -> dict:
    ...
    sync = _sync_results(f"[{OLLAMA_MODEL}] Add chapter {chapter_number}: {title}")
    if sync:
        result["git_sync"] = sync
    return result
