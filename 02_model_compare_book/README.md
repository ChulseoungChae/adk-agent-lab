...
![02_model_compare_book 동작 개요](img/overview.png)

**그림 설명:** 동일한 주제의 책을 gemma4·qwen3 등 **4개 모델이 순서대로** 집필합니다. 모델마다 별도 폴더에 결과가 쌓이고, GPU 사용량도 함께 기록해 성능·품질을 비교합니다.

## 어떻게 동작하나요?

1. `run.py compare`가 4개 모델을 **순차 실행**합니다.
2. 각 모델마다 ADK 에이전트가 플랫폼 데이터를 조사하고 한국어 기술서를 집필합니다.
3. 결과는 `output/{모델명}/` 폴더에 **모델별로 분리** 저장됩니다.
4. 집필 중 **GPU 모니터**가 5초마다 사용량을 CSV에 기록하고, 작업 종료 시 min/max/avg를 요약 로그에 남깁니다.
5. 전체 비교 결과는 `comparison_summary.json`에 소요 시간·챕터 수·GPU 통계가 정리됩니다.
