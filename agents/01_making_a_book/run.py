#!/usr/bin/env python3
"""01_making_a_book — 책 작성 ADK 에이전트 실행 스크립트.
...
book_tools = importlib.import_module(f"{AGENT_NAME}.book_tools")
config = importlib.import_module(f"{AGENT_NAME}.config")
session_log = importlib.import_module(f"{AGENT_NAME}.session_log")
...
def _run_adk(mode: str, extra_args: list[str] | None = None) -> None:
    cmd = ["adk", mode, AGENT_NAME]
    if extra_args:
        cmd.extend(extra_args)
    subprocess.run(cmd, check=True, env=_adk_env(), cwd=str(AGENTS_ROOT))
